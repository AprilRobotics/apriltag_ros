...
bool get_ROS_parameter(ros::NodeHandle& pnh, std::string name, double& param) {
...
int main(int argc, char **argv) {
  ...
  ros::ServiceClient client = nh.serviceClient<apriltags2_ros::AnalyzeSingleImage>("single_image_tag_detection");

  // Get the request parameters
  apriltags2_ros::AnalyzeSingleImage service;
  service.request.full_path_where_to_get_image = apriltags2_ros::apriltag_getopt<std::string>(pnh, "image_load_path", "");
  ...
  double fx, fy, cx, cy;
  if (!get_ROS_parameter(pnh, "fx", fx))
    return 1;
  if (!get_ROS_parameter(pnh, "fy", fy))
    return 1;
  if (!get_ROS_parameter(pnh, "cx", cx))
    return 1;
  if (!get_ROS_parameter(pnh, "cy", cy))
    return 1;
  ...
  // Call the service (detect tags in the image specified by the image_load_path)
